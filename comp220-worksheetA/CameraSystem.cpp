...
void CameraSystem::SendCameraData(std::unique_ptr<Coordinator>& coord, GLint _view, GLint _projection)
{
...
void CameraSystem::UpdateCameraPosition(std::unique_ptr<Coordinator> &coord)
{
	for (Entity ent : sy_Entities)
	{
		CameraComponent cam = coord->GetComponent<CameraComponent>(ent);
